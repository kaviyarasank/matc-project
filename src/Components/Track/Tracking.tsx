    ...
    return (
        <div className="trackingDiv">
            <div className="container">
                <button onClick={handle} className="border-0 icon-styleTrack ms-3 choicesbutton">Print Receipt</button>
    <article className="card mt-3">
        <header className="card-header"> My Orders / Tracking </header>
        <div className="card-body">
            <h6>Order ID: {address?.created}</h6><h6>Your Total Amount: ${amount}{" "}({address?.card?.funding})</h6>
            <article className="card">
                <div className="card-body row">
                    <div className="col"> <strong>Estimated Delivery time:</strong> <br/> {myFutureDate.toString()} </div>
                    <div className="col"> <strong>Delivery Address:</strong> <br/> {address?.card?.name},<br/>{address?.card?.address_line1},<br/>{address?.card?.address_city},{address?.card?.address_zip}<br/>{address?.card?.address_country}, | <i className="fa fa-phone"></i> {profile?.mobileNo}.</div>
                    <div className="col"> <strong>Status:</strong> <br/> Picked by the courier </div>
                    <div className="col"> <strong>Tracking Id:</strong> <br/> {address?.id} </div>
                </div>
            </article>
            {<div className="track">
            ...
            <ul className="row">
                ...
                <li className="col-md-4">
                    <figure className="itemside mb-3">
                        <div className="aside"><img src={data?.image} className="img-sm border" alt=""/></div>
                        <figcaption className="info align-self-center">
                            <p className="title">{data?.name.slice(0,50)}</p> <span className="text-muted">{data?.price_string} </span>
                        </figcaption>
                    </figure>
                </li>
                )
            ...
            <button className="btn btn-warning choicesbutton" data-abc="true" onClick={backToOrder}> <i className="fa fa-chevron-left"></i> Back to orders</button>
 </div>
 </article>
                ...
                <ModalBody className="modalBodyPopUp">
                  <div className="modalpopRemit-wrapper">
                    <div className="modalpopRemit-title">
                        ...
                        <button onClick={Print} className="border-0 choicesbutton">Print Receipt</button>
            ...
            <div className="panel-body p30">
                <div className="row">
                    <div className="col-lg-6">
                        <div className="invoice-logo"><img width="100" className="invoiceBill" src={timezone} alt="Invoice logo"/></div>
                    </div>
                    ...
                    <div className="invoice-to mt25">
                            ...
                            {address?.card?.name},<br/>{address?.card?.address_line1},<br/>{address?.card?.address_city},{address?.card?.address_zip}<br/>{address?.card?.address_country}, | <i className="fa fa-phone"></i> {profile?.mobileNo}.
                            </ul>
                        </div>
                    <div className="col-lg-12">
                        ...
                        <div className="invoice-items">
                            ...
                            return (
                                        <tr>
                                            ...
                                            <td className="text-center">{data?.price_string}</td>
                                        </tr>
                            )})}
                        ...
                        <div className="invoice-footer mt25">
                            <p className="text-center">Generated on {myCurrentDate.toString()}</p>
                        </div>
                    </div>
                </div>
            </div>
